    ...
    <script>
        ...
        $(document).ready(function() {
            $('#province').change(function() {
                ...
                $.ajax({
                    url: '{{ url("/getAmphoe/") }}/'+province_id,
                    type: 'post',
                    dataType: 'json',
                    success: function(respose) {
                        console.log(respose);

                        //$('#amphoe').find('option:not(:first)').remove();

                        if (respose['amphoes'].length > 0) {
                            $.each(respose['amphoes'], function(key, value) {
                                $('#amphoe').append("<option value=" + value['id'] + "> " +
                                    value['name'] + " </option>")
                            });
                        }
                    }
